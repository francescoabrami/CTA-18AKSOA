...
In questa ottava esercitazione di Controlli Automatici vengono affrontati temi fondamentali relativi al progetto di controllori digitali per sistemi dinamici, con particolare riferimento all’applicazione concreta su un modello di motore elettrico.

L’esercitazione consente di mettere in pratica le procedure di progetto, discretizzazione, simulazione e verifica dei controllori digitali, consolidando la comprensione delle tecniche di conversione dal dominio continuo a quello discreto. Viene inoltre dato ampio spazio all’uso di strumenti di simulazione \textsc{Matlab}/\textsc{Simulink}, indispensabili in ambito industriale per l’analisi e la progettazione assistita da calcolatore di sistemi di controllo automatico.
...
Il sistema complessivo, avente come ingressi il comando $u(t)$ e la coppia di disturbo esterna $T_d(t)$ e come uscita la tensione $V_\omega(t) = K_t \cdot \omega(t)$ della dinamo tachimetrica (usata per misurare la velocità angolare $\omega$), può essere rappresentato in \textsc{Simulink} mediante il seguente modello:\\
...
Infine si richiede di verificare in ambiente \textsc{Matlab}/\textsc{Simulink} il soddisfacimento delle specifiche, secondo i seguenti passi indicati di seguito:
...
Per questa verifica (sul comando $u$) si richiede di utilizzare il seguente schema \textsc{Simulink}:
...
\subsubsection*{Soluzione esercizio \ref{progetto1}}
...
\subsubsection*{Soluzione esercizio \ref{progetto2}}

Procediamo ora con la soluzione dell'esercizio di cui abbiamo appena riportato il testo.
